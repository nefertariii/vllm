#!/bin/bash
# Rate sweep — Gemma-3-4B-IT, Both (PA eviction + PA sched)
# GPU 3 (A5000), port 8766
...
LOG=$OUTDIR/both_${TS}.log

echo "=== Rate sweep — Gemma-3-4B-IT Both (GPU 3) ===" | tee "$LOG"
...
start_server() {
    echo "" | tee -a "$LOG"
    echo ">>> [$(date +%H:%M:%S)] Starting Both server (rate=$1)" | tee -a "$LOG"
...
run_client() {
    ...
    local out=$OUTDIR/Both_rate${rate_str}_${TS}.json
    echo ">>> [$(date +%H:%M:%S)] Client: Both  rate=$rate req/s" | tee -a "$LOG"
    cd benchmarks

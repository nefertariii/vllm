#!/bin/bash
# 4-policy benchmark — Llama-3.1-8B-Instruct on A5000 (combined-prefix-aware branch)
#
# Policies:
...
#   hf_max_len=49850, hf_output_len=150, rate=inf, ignore-eos
#
# Usage:
#   tmux new-session -d -s bench_llama_combined \
#     "bash run_llama_8b_combined_benchmark.sh 2>&1 | tee bench_llama_combined.log"
...
OUTDIR=results_arxiv/llama_8b_combined
PORT=8767
...
echo "=== 4-policy benchmark — Llama-3.1-8B-Instruct (combined branch) ===" | tee "$LOG"
